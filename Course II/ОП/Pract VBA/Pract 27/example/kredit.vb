...
Private Sub CommandButton11_Click() 'последяя
    rs.MoveLast
    ShowRecord
End Sub
...
Private Sub CommandButton4_Click() 'поиск
 ...
 Else
     If (IsNumeric(Lname) = True) Then
         ...
         criteria = "[Номер кредита] = '" & Lname & "'" ' поиск по 
номеру
         ff = 0
         fff = adSearchForward
         rs.MoveFirst
        ...
        End If
        End If
         TextBox6.Text = Empty ' очищаем текстбокс
...
Private Sub CommandButton7_Click() ' удалить
...
Private Sub CommandButton8_Click() 'документ для печати
Dim sum, itog As Integer
Dim i As Document
sum = CInt(TextBox4.Text) 'сумма берется из текстбокс
itog = sum * 1.1 ' умажаем на годовую ставку(10%)

Set i = 
Application.Documents.Add("C:\Users\Дос\Desktop\27\dogovor.docx")
...
rs.Source = "SELECT Кредит.* FROM Кредит;"
...
Private Sub ShowRecord() '  заполнение тексбоксы из записей
    ...
    TextBox5.Text = rs.Fields("Номер кредита").Value
End Sub
Private Sub ShowEmptyRecord() '  обнуление
    ...
    TextBox2.Text = Empty
      ...
      TextBox5.Text = Empty
End Sub
Private Sub dobav() '  добавляем в засписи из тексбокса
...
rs.Fields("Номер кредита").Value = TextBox5.Text
End Sub
