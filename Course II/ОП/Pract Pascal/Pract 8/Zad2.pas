...
writeln('а) названия телепередач, которые идут в указанный день 
в указанный промежуток времени');
writeln('б) на каком канале и в какое время будет 
транслироваться развлекательная передача «Поле чудес»');
writeln('в) названия телепередач, транслирующихся в указанное 
время на разных каналах');
writeln('г) название передачи, завершающей эфир в каждый день 
недели');
write('=> '); read(k);
        ...
        while not eof(TV) do
          begin
            ...
            if (f[i].name = 'Поле чудес') then
                writeln('Поле чудеc будет на канеле 
№',f[i].chan,' в ',f[i].timeHH,':',f[i].timeMM,', 
',f[i].dayname);
            i:=i+1;
          end;
     end;
