...
begin
writeln('Используя разработанный модуль matrix, решить следующие задачи:');
writeln('a) систему линейных уравнений N-го порядка (2<= N <= 10) методом Крамера');
writeln('б) отсортировать заданный массив меличин типа Matrica в порядке возрастания значений определителей матриц');
writeln('в) найти матрицу С, равную A*B-B*A, если заданы величины A и B типа Matrica');
write('=> '); read(flag);
case flag of
  'а' : begin
