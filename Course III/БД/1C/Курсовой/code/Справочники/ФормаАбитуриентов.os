...
Процедура СоздатьДокументЗаявлениеНаПоступление()
   ...
   НовоеЗаявление.СпециальностьАбитуриента = Объект.СпециальностьАбитурента;
   НовоеЗаявление.СреднийБаллАбитуриента = Объект.СреднийБаллАбитуриента;
   НовоеЗаявление.ТипАттестатаАбитуриента  = Объект.ТипАттестатаАбитуриента;
   НовоеЗаявление.ПриоритетЗачисленияАбитуриента = Объект.ПриоритетЗачисленияАбитуриента;
   НовоеЗаявление.ФормаОплатыАбитуриента = Объект.ФормаОплатыАбитуриента;
   НовоеЗаявление.АдресАбитуриента = Объект.АдресАбитуриента;
   НовоеЗаявление.ДатаРожденияАбитуриента = Объект.ДатаРожденияАбитуриента;
   ...
   ОбъектРодителя = Справочники.РодителиАбитуриентов.НайтиПоНаименованию(ФИОРодителя);
   НовоеЗаявление.ДатаРожденияРодительАбитуриента = ОбъектРодителя.ГодРождения;
   ...
   Для предмет = 0 По Объект.ОценкиАбитуриента.Количество()-1 Цикл
		НоваяСтрокаЗаписи = НовоеЗаявление.ОценкиАбитуриента.Добавить();
   ...
   Для документ = 0 По Объект.ДокументыАбитуриента.Количество()-1 Цикл
		НоваяСтрокаЗаписи = НовоеЗаявление.ДокументыАбитуриента.Добавить();
...
Процедура ПроверкаОтчетTelegram()
	Если Справочники.Уведомления.НайтиПоРеквизиту("ФИО",Объект.Наименование).Пустая() Тогда
		...
		Сообщение = "<b>*Добавление нового абитуриента*</b>"+Символы.ПС+Символы.ПС+
				...
				"Специальность: "+Объект.СпециальностьАбитурента+Символы.ПС+Символы.ПС+
				...
				"Родитель: "+Объект.РодительАбитуриента+Символы.ПС+
		...
		СоздатьОтчетTelegram(Сообщение,Объект.Наименование,"Добавление нового абитуриента",0);
...
Процедура ПроверкаЗаявлениеНаПоступление()
	Если Документы.ЗаявлениеНаПоступление.НайтиПоРеквизиту("ФИОАбитуриента", Объект.Наименование).Пустая() Тогда
...
Процедура УстановитьКартинку(ИмяФайла)
	...
	Если Справочники.Уведомления.НайтиПоРеквизиту("ФИО",НовыйФайлКартинки.Наименование).Пустая() Тогда
		
		Сообщение = "<b>*Добавление нового файла*</b>"+Символы.ПС+Символы.ПС+
				"Название: "+НовыйФайлКартинки.Наименование+Символы.ПС+
				"Дата добавления: "+НовыйФайлКартинки.Дата+Символы.ПС;
								
		СоздатьОтчетTelegram(Сообщение,НовыйФайлКартинки.Наименование,"Добавление нового файла",2);
...
&НаСервере
Процедура СортировкаАбитуриентовСправочникСервер()
	СортировкаАбитуриентовСправочник();
КонецПроцедуры

&НаКлиенте
Процедура Сортировка(Команда)
	СортировкаАбитуриентовСправочникСервер();
КонецПроцедуры

&НаКлиенте
Процедура УдалениеАбитуриента(Команда);
	//Объект.Наименование
	УдалениеАбитуриентаНаСервере(Объект.Наименование);
	ЭтаФорма.Закрыть();
КонецПроцедуры

&НаСервере
Процедура УдалениеАбитуриентаНаСервере(ТекущееНаименование)
	
	ДокументОВозвратеДокумнтовОбращение = Документы.ДокументОВозвратеДокументов.НайтиПоРеквизиту("ФИОАбитуриентаВозврат",Объект.Наименование);
	ДокументЗаявлениеНаПоступлениеОбращение = Документы.ЗаявлениеНаПоступление.НайтиПоРеквизиту("ФИОАбитуриента",Объект.Наименование);
	
	ТекущийОбъект = Справочники.Абитуриенты.НайтиПоНаименованию(ТекущееНаименование).ПолучитьОбъект();
	Если Объект.ПометкаУдаления = Ложь Тогда
		Если ДокументОВозвратеДокумнтовОбращение.Пустая() Тогда
			НовыйДокументВозврата = Документы.ДокументОВозвратеДокументов.СоздатьДокумент();
			НовыйДокументВозврата.Дата = ТекущаяДата();
			НовыйДокументВозврата.ФИОАбитуриентаВозврат = Объект.Наименование;
			НовыйДокументВозврата.ДатаРожденияВозврат = Объект.ДатаРожденияАбитуриента;
			НовыйДокументВозврата.АдресАбитуриентаВозврат = Объект.АдресАбитуриента;
			НовыйДокументВозврата.ТелефонАбитуриентаВозврат = Объект.ТелефонАбитуриента;
			НовыйДокументВозврата.EmailАбитуриентаВозврат = Объект.EmailАбитуриента;
			
		   Для документ = 0 По Объект.ДокументыАбитуриента.Количество()-1 Цикл
				НоваяСтрокаЗаписи = НовыйДокументВозврата.СписокДокументов.Добавить();
				НоваяСтрокаЗаписи.Документ = Перечисления.ПеречислениеДокументыАбитуриента.Получить(документ);			
				НоваяСтрокаЗаписи.Номер = Объект.ДокументыАбитуриента[документ].Номер;
		   КонецЦикла;			НовыйДокументВозврата.Записать(РежимЗаписиДокумента.Проведение);
		КонецЕсли;
		Если Не ДокументЗаявлениеНаПоступлениеОбращение.Пустая() Тогда
	ДокументЗаявлениеНаПоступлениеОбращение.ПолучитьОбъект().Удалить();
		КонецЕсли;
	...
	Иначе
		...
		ТекущийОбъект.Записать();
		Если Не ДокументОВозвратеДокумнтовОбращение.Пустая() Тогда
	ДокументОВозвратеДокумнтовОбращение.ПолучитьОбъект().Удалить();
		КонецЕсли;
		Если ДокументЗаявлениеНаПоступлениеОбращение.Пустая() Тогда
			СоздатьДокументЗаявлениеНаПоступление();	
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
