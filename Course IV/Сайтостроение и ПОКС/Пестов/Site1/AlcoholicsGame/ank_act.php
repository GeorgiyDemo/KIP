...
//$_COOKIE['hp_user']=$hp_user; 
$_SESSION['hp_user']=$hp_user; 
$_SESSION['error_w']=$error_wr;
 if(isset($_POST['ank_submit']))//Проверяет отправлен ли запрос
{
     $id=0;
...
if (file_exists($ank_filename))
    ...
    $lines = file($ank_filename); $lastLine = $lines[count($lines)-1];
    ...
    else{echo"<h1 color='red'><strong>Нет доступа к файлу. Файл не доступен для записи</strong></h1>";}
    {
  //$arr_m .=$buffer."<br/>";
    }
...
}else   { echo '<h1><strong>Нет файла</strong></h1>';}
